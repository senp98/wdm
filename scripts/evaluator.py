...
def calculate_fids(ref_batch, sample_batches):
    config = tf.ConfigProto(
        allow_soft_placement=True  # allows DecodeJpeg to run on CPU in Inception graph
    )
    config.gpu_options.allow_growth = True
    ...
    print("computing/reading reference batch statistics...")
    ref_stats, _ = evaluator.read_statistics(ref_batch, ref_acts)
    fids=[]
    for i in range(len(sample_batches)):
        sample_batch=sample_batches[i]
        print(f"computing {i}th sample batch activations...")
